...
if (
  ...
  try {
    ...
    console.warn('⚠️ Firebase init failed, running in demo mode');
  }
} else {
  console.warn('⚠️ Firebase env missing, running in demo mode');
}
...
const authMiddleware = async (req, res, next) => {
  try {
    ...
    let role = 'student';
    try {
      const profile = await sheetsService.getUserProfileByEmail(decoded.email);
      role = profile?.role || role;
    } catch (e) {}
